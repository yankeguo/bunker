...
- `database.go` — SQLite (gorm, pure-Go driver `glebarez/sqlite`) setup and `users.yaml` seeding
- `static.go` — embeds `ui/.output/public`; proxies to `localhost:3000` when `DEBUG=ui`
